...
while (gr2.next()) {
	if (String(gr2.u_dealer.u_dealer_number).length == 5) {
		...
		if (gr3.getRowCount() == 0) {
			arr2_0.push(String(gr2.number) + ": " +String(gr2.u_dealer.u_dealer_number));
			count_0++;
		} else if (gr3.getRowCount() == 1 && gr3.next()) {
			arr2_1.push(String(gr2.number) + ": " +String(gr2.u_dealer.u_dealer_number)+" ("+String(gr3.u_dealer_number)+")");
			count_1++;
		} else {
			var tmp = [];
			while (gr3.next()) {
			...
			arr2_2.push(String(gr2.number) + ": " + String(gr2.u_dealer.u_dealer_number)+" ("+tmp.join(",")+")");
			count_2++;
		}
